...
with app.app_context():
    drop_table = text('DROP TABLE IF EXISTS users;') # - drop users table if exists, create new users table and add some rows for sample
    users_table = text(""" 
    CREATE TABLE users(
    ...
    VALUES
        ...
        """)
    db.session.execute(drop_table) # - Execute sql commands and commit them
